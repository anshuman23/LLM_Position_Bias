...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default="cnn", help="cnn/xsum/reddit/news")
...
if args.model == 'cnn':      #USE
    model="facebook/bart-large-cnn"

elif args.model == 'xsum':   #USE
    model="facebook/bart-large-xsum"

elif args.model == 'news':   #USE
    model="fine-tuned/bart-large-news-summary/checkpoint-5000"
    
elif args.model == 'reddit':   #USE
    model="fine-tuned/bart-large-reddit_tifu_long/checkpoint-12000"  
...
pipe = pipeline("summarization",model = model,device=args.device)
...
plt.title("Segment vs frequency [{}-BART]".format(args.dataset))
...
plt.xticks(x)
plt.legend()
plt.savefig('results/BART-{}-{}.png'.format(args.model,args.dataset))
...
df.to_csv('results/BART-{}-{}.csv'.format(args.model,args.dataset))

data.save_to_disk('saved_models/BART-{}-{}'.format(args.model,args.dataset))
