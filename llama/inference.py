...
Return a summary comprising of 1 sentence. With the sentence in a bulleted format.
...
def call(generator, articles, name, max_gen_len,temperature,top_p):
    ...
    for result in results:
        ...
        if name=='cnn_dailymail':
            final.append(prompt_output_parse_CNN(result['generation']['content']))

        else:
            final.append(prompt_output_parse(result['generation']['content']))
...
def main(
    ...
    res=call(generator,dataset[article_key], name,max_gen_len,temperature,top_p)
