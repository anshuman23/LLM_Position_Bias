...
if args.model == 'cnn':      #USE
    ...
    model="fine-tuned/pegasus-large-news-summary/checkpoint-1000"
...
plt.title("Segment vs frequency [{}-{}-Pegasus]".format(args.model,args.dataset))
...
plt.xticks(x)
plt.legend()
plt.savefig('results/pegasus-{}-{}.png'.format(args.model,args.dataset))
...
df.to_csv('results/pegasus-{}-{}.csv'.format(args.model,args.dataset))

data.save_to_disk('saved_models/pegasus-{}-{}'.format(args.model,args.dataset))
